...
export async function renderInvoicePDF(order: ClientOrderDetail) {
  ...
  // -------------------- Invoice Meta --------------------
  const invoiceNo = `INV${String(order.orderNumber ?? order._id)
    .replace(/\D+/g, "")
    .slice(-6)
    .padStart(6, "0")}`;
  ...
  const meta: [string, string][] = [
    ...
    ["Invoice Date:", date],
    ["Order No:", order.orderNumber || `WNR_${String(order.orderNumber ?? order._id).replace(/\D+/g, "").slice(-4).padStart(4, "0")}`],
    ["Order Date:", date],
  ...
  const grand = Number(
    typeof order.total === "number" ? order.total : sub + shipCost
  );
...
// -------- Helpers --------
function formatAddr(a: ClientOrderDetail["addressSnapshot"]) {
  if (!a) return "—";
